...
APP_FOLDER = os.path.join(os.path.dirname(os.path.realpath(sys.argv[0])), ".\\")
...
class MineSweeperGUI(superGUI.Ui_MainWindow):
    def __init__(self, MainWindow):
        ...
        pygame.mixer.init()
        pygame.mixer.music.load(os.path.join(APP_FOLDER, file))
        ...
        self.mainWindow.setWindowIcon(QIcon(os.path.join(APP_FOLDER, "media/mine.jpg")))
        ...
        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/underway.png"))
    ...
    def mineAreaRightRelease(self, i, j):
        if not self.finish:
            if self.mineLabel[i][j].status == 0:
                pixmap = QPixmap(os.path.join(APP_FOLDER, "media/flag.jpg"))
                ...
                self.mineLabel[i][j].setPixmap(QPixmap(os.path.join(APP_FOLDER, "media/question.jpg")))
            ...
            for j in i:
                if j.num == -1 or j.status == 2:
                    ...
                    if j.num == -1 and j.status == 2:
                        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/correct.jpg"))
                    elif j.num == -1:
                        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/mine.jpg"))
                    else:
                        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/mistake.jpg"))
                    j.setPixmap(pixmap)
    ...
    def gameWin(self):
        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/win.png"))
        self.label_2.setPixmap(pixmap)
        self.label_2.setScaledContents(True)
        self.gameFinished()
        try:
    ...
    def gameFailed(self):
        pixmap = QPixmap(os.path.join(APP_FOLDER, "media/fail.png"))
        self.label_2.setPixmap(pixmap)
        self.label_2.setScaledContents(True)
        self.gameFinished()
